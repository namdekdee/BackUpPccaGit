	...
	$('document').ready(function() {
		...
		$("#btnAddCaList")
				.click(	function(e) {
						...
						next = next + 1;
						var newIn = '<div id="form'+ next +'"> <div class="row"><div class="form-group" ><label id="lblCaCompany" class="col-sm-3 control-label" required>เบิกจากบริษัท</label><div class="col-sm-6"><select id="ddlCompany" class="form-control" style="width: 100%;"	data-placeholder="เลือกบริษัท"><option value="PTG">PTG พีทีจี เอ็นเนอยี</option><option value="PTC">PTC พีทีซี</option><option value="PUN">PUN กาแฟพันธุ์ไทย</option>	<option value="PTGLG">PTGLG พีทีจี โลจิสติก</option></select></div>	</div></div><div class="row"><div class="form-group" ><label id="lblCaAmount" class="col-sm-3 control-label">จำนวนเงิน </label><div class="col-sm-2"><input type="text" id="inputCaAmount" class="form-control" placeholder="ระบุ..."></div></div></div></div>';
						var newInput = $(newIn);
						var removeBtn = '<div class="col-sm-offset-5"><button id="remove'+ (next - 1)+ '" class="btn btn-danger remove-me center-block" >Remove</button><br></div></div><div id="addList">';
